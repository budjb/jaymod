...
qboolean BG_ParseCharacterFile( const char *filename, bg_characterDef_t* characterDef )
{
	...
	while( 1 ) {
		...
		} else if( !Q_stricmp( token.string, "undressedModel" ) ) {
			if( !PC_String_ParseNoAlloc( handle, characterDef->undressedModel, sizeof(characterDef->undressedModel) ) ) {
				return BG_PCF_ParseError( handle, "expected undressedModel filename" );
			}
		} else {
