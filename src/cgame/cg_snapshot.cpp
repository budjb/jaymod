...
#define RATESTEP 1000
#define RATEUPDATEDELAY 5000

void CG_AdjustRate()
{
    // Do nothing if not enabled
    if (!cg_autoRate.integer)
        return;

    // Do nothing if disallowed
    if (!cg.allowIncrease)
        return;

    // Only allow this to run in 5 second intervals
    if (cg.rateIncreaseTime > cg.time)
        return;

    // 2 means set to server max
    if (cg_autoRate.integer == 2) {
        if (cvars::cl_rate.ivalue != cvars::sv_maxRate.ivalue)
            cvars::cl_rate.set( cvars::sv_maxRate.ivalue );
    } else {
        // No increase request?  Bail.
        if (!cg.requestRateIncrease)
            return;

        // Reset the request
        cg.requestRateIncrease = qfalse;

        // We're capped.
        if (cvars::cl_rate.ivalue >= cvars::sv_maxRate.ivalue)
            return;

        // New rate, cap if need be
        int newrate = cvars::cl_rate.ivalue + RATESTEP;
        if (newrate > cvars::sv_maxRate.ivalue)
            newrate = cvars::sv_maxRate.ivalue;

        // Set the new rate
        cvars::cl_rate.set( newrate );
    }

    // Mark the time
    cg.rateIncreaseTime = cg.time + RATEUPDATEDELAY;
}
...
static snapshot_t *CG_ReadNextSnapshot( void ) {
	...
	while ( cgs.processedSnapshotNum < cg.latestSnapshotNum ) {
		...
		if ( r ) {
            ...
            // Jaybird - track supressed snapshots so that we can increase the rate
            if (dest->snapFlags & SNAPFLAG_RATE_DELAYED && cg.rateIncreaseTime < cg.time)
                cg.requestRateIncrease = qtrue;
