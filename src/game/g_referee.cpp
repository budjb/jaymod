...
//	|| cvars::gameState.ivalue == GS_INTERMISSION) {
...
void G_refMute_cmd(gentity_t *ent, qboolean mute)
{
	...
	if( mute ) {
        string muter = ent ? ent->client->pers.netname : "referee";

        G_MutePlayer(g_entities + pid, muter);
...
{ 
	...
	if (cnum != MAX_CLIENTS ) {
		if( level.clients[cnum].sess.referee != RL_RCON ) {
            G_MutePlayer(g_entities + cnum, "console");
...
void G_refPrintf( gentity_t* ent, const char *fmt, ... )
{
	...
	if(ent == NULL) trap_Printf(va("%s", text));	// CHRUKER: b047 - Added linebreak to the string
