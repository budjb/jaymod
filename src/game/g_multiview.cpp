...
qboolean G_smvRunCamera(gentity_t *ent)
{
	...
	if(tps->stats[STAT_PLAYER_CLASS] == PC_ENGINEER) chargeTime = g_engineerChargeTime.integer;
	else if(tps->stats[STAT_PLAYER_CLASS] == PC_MEDIC) chargeTime = g_medicChargeTime.integer;
	else if(tps->stats[STAT_PLAYER_CLASS] == PC_FIELDOPS) chargeTime = g_LTChargeTime.integer;
	else if(tps->stats[STAT_PLAYER_CLASS] == PC_COVERTOPS) chargeTime = g_covertopsChargeTime.integer;
	else chargeTime = g_soldierChargeTime.integer;

	chargeTime = int( (level.time - tps->classWeaponTime >= (int)chargeTime) ? 0 : (1 + floor(15.0f * (float)(level.time - tps->classWeaponTime) / chargeTime)) );
	sprintTime = int( (ent->target_ent->client->pmext.sprintTime >= 20000) ? 0.0f : (1 + floor(7.0f * (float)ent->target_ent->client->pmext.sprintTime / 20000.0f)) );
	weapHeat   = int((float)tps->curWeapHeat * 15.0f / 255.0f);
	hintTime   = (tps->serverCursorHint != HINT_BUILD && (tps->serverCursorHintVal >= 255 || tps->serverCursorHintVal == 0)) ?
														0 : int(1 + floor(15.0f * (float)tps->serverCursorHintVal / 255.0f));
