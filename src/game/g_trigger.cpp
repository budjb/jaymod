...
void heal_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	for( i = 0; i < clientcount; i++) {
		...
		//G_ResetPoisonEvents( touchClients[i] );
		G_ResetOnePoisonEvent( touchClients[i] );
...
// Jaybird
#define HEALTH_REGENTIME g_healthRechargeTime.integer
...
void ammo_touch( gentity_t *self, gentity_t *other, trace_t *trace ) {
	...
	// Jaybird - trying to keep ammo in g_knifeonly
	...
	if(self->health == -9999) {
		...
		count = int( min( float(clientcount), self->health / float(self->damage) ));
	}
...
// Jaybird
#define AMMO_REGENTIME g_ammoRechargeTime.integer
...
void Touch_flagonly (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if ( ent->spawnflags & RED_FLAG && other->client->ps.powerups[ PW_REDFLAG ] ) {
		...
		AddScore( other, int(ent->accuracy) ); // JPW NERVE set from map, defaults to 20
